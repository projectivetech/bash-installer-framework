...
function assert_not() {
  if [ $# -ne 1 ]
  then
    assert_fail "assert_not called with wrong number of parameters!"
  fi
...
function assert_variable_exists() {
  if [ $# -ne 1 ]
  then
    assert_fail "assert_variable_exists called with wrong number of parameters!"
  fi

  local gvar=$1
  if [[ -z ${!gvar} ]]; then
    assert_fail "variable ${gvar} isn't set"
  fi
}
...
function assert_file() {
  if [ $# -ne 1 ]
  then
    assert_fail "assert_file called with wrong number of parameters!"
  fi
